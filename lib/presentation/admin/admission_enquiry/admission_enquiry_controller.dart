...
class AdmissionEnquiryController extends GetxController {
  ...
  RxList<AdmissionConfirmation> admissionEnquiryList =
  ...
  Future<void> callService() async {
    ...
    if (await CU.checkInternet()) {
      resJson = await ApiClient.call(
        ...
        apiUrl:
            "https://erp.triz.co.in/admission/admission_enquiry?type=API&token=${userInfo[CS.token]}&sub_institute_id=${userInfo[CS.sub_institute_id]}&syear=${userInfo[CS.syear]}",
    ...
    if (resJson[CS.status].toString() == StatusCode.Success.toString()) {
      ...
      admissionEnquiryList.value += model.data!;
  ...
  Future<void> callDeleteOutWard({required int id, required int index}) async {
    ...
    admissionEnquiryList.removeAt(index);
  }
}
