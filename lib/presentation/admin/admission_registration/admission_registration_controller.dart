...
class AdmissionRegistrationController extends GetxController {
  Content data = Get.arguments[0];
  Map<String, dynamic> userInfo = Get.arguments[1];

  RxBool isLoading = false.obs;

  RxList<AdmissionConfirmation> admissionRegistrationList =
  ...
  Future<void> callOutwardService() async {
    ...
    if (await CU.checkInternet()) {
      resJson = await ApiClient.call(
        ...
        apiUrl:
            "https://erp.triz.co.in/admission/admission_registration?type=API&token=${userInfo[CS.token]}&sub_institute_id=${userInfo[CS.sub_institute_id]}&syear=${userInfo[CS.syear]}",
    ...
    if (resJson[CS.status].toString() == StatusCode.Success.toString()) {
      ...
      admissionRegistrationList.value += model.data!;
  ...
  Future<void> callDeleteOutWard({required int id, required int index}) async {
    ...
    admissionRegistrationList.removeAt(index);
  }
}
