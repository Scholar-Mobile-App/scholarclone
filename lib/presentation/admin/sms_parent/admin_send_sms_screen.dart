...
import 'package:scholar_clone/presentation/admin/sms_parent/admin_send_sms_controller.dart';
...
class AdminSendSMSScreen extends StatelessWidget {
  ...
  final AdminSendSMSController _controller = Get.put(AdminSendSMSController());

  @override
  Widget build(BuildContext context) {
    return Obx(
      () => Scaffold(
        appBar: teacherAppBar(text: "Send SMS"),
        body: Stack(
          children: [
            Container(
              width: Get.width,
                    ...
                    onChanged: (_) {
                      if (!_controller.selectAll.value) {
                        ...
                        for (int i = 0;
                          ...
                          _controller.keyList.add(
                              'mobile_number[${_controller.studentList[i].id!}]');
                          _controller.valueList
                              .add(_controller.studentList[i].mobile!);
                        }
                      } else {
                ...
                child: Checkbox(
                  ...
                  onChanged: (_) {
                    ...
                    if (!_controller.checkData.contains(list.enrollmentNo)) {
                      ...
                      _controller.keyList.add('mobile_number[${list.id}]');
                      _controller.valueList.add(list.mobile!);
                    } else {
                      _controller.checkData.remove(list.enrollmentNo);
                      _controller.email.remove(list.email!);
                      _controller.keyList.remove('mobile_number[${list.id}]');
                      _controller.valueList.remove(list.mobile!);
                    }
                  },
                ),
              ),
