...
class TWRTProgressReportController extends GetxController {
  ...
  Future<void> callServiceStandared(int sectionId) async {
    stdName.value = [];
    Map<String, dynamic> body = <String, dynamic>{
      CS.token: userInfo[CS.token],
      CS.sub_institute_id: userInfo[CS.sub_institute_id],
    ...
    if (await CU.checkInternet()) {
      resJson = await ApiClient.call(
        ...
        apiUrl: userInfo["user_profile_name"] == "Admin"
            ? "https://erp.triz.co.in/get_adminStudentList"
            : "https://erp.triz.co.in/allStudentListAPI",
        isShowProgressDialog: false,
      );
    } else {
