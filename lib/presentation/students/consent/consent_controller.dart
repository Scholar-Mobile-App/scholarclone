...
import 'package:scholar_clone/model/student/consent_model.dart';
...
class ConsentController extends GetxController {
  ...
  RxList<Consent> consentList = <Consent>[].obs;
  ...
  Future<void> callService() async {
    ...
    if (await CU.checkInternet()) {
      resJson = await ApiClient.call(
        Get.context!,
        body: body,
        apiUrl: "https://erp.triz.co.in/consentListAPI",
    ...
    if (resJson["status_code"].toString() == StatusCode.Success.toString()) {
      ConsentModel model = ConsentModel.fromJson(resJson);

      for (var element in model.data!) {
        consentList.add(element);
      }
